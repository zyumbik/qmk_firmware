...
static const int led_num[] = {126,12,182,158,204,218,250,14,254,222};
...
int currentNum = 0;
static int length = sizeof(led_num) / sizeof(int);

/**/
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    if (record->event.key.col==0 && record->event.key.row==0 && record->event.pressed) {
        shift_out((uint8_t)led_num[currentNum]);
        currentNum = (currentNum + 1) % length;
    }
    return true;
}
